...
| Durable execution | **Step Functions Standard for the scheduled monitor only** | The nightly watchlist run is a long-running, partially-failing, auditable workflow — exactly Step Functions' shape, and it's already in the CDK stack's ecosystem. Interactive chat needs no checkpointer; a turn is seconds, and transcripts make sessions reconstructable. |
...
Secrets: `ANTHROPIC_API_KEY` lands in the Fargate task via Secrets Manager in the CDK API stack (one line in [CICD-PLAN.md](CICD-PLAN.md)'s api-stack); never shipped to the browser.
